...
class Chat{
    ...
    private $user  = 'root';
    private $password   = "";
    private $database  = "mental_health_app";      
    private $chatTable = 'chat';
	private $chatUsersTable = 'chat_users';
	private $chatUsersTable1 = 'VIEW_for_chat';
	private $chatUsersTable2 = 'VIEW_for_chat1';	
 ...
 public function __construct(){
	...
	public function chatUsers($userid){
		$sqlQuery = "
			SELECT * FROM ".$this->chatUsersTable1." 
			WHERE User_ID = '$userid'";
		return  $this->getData($sqlQuery);
	}
	public function chatUsers1($userid){
		$sqlQuery = "
			SELECT * FROM ".$this->chatUsersTable2." 
	...
	public function getUserChat($from_user_id, $to_user_id) {
		...
		foreach($userChat as $chat){
			...
			if($chat["sender_userid"] == $from_user_id) {
				...
				$conversation .= '<img width="22px" height="22px" src="images/'.$fromUserAvatar.'" alt="" />';
			} else {
				$conversation .= '<li class="replies">';
				$conversation .= '<img width="22px" height="22px" src="images/'.$toUserAvatar.'" alt="" />';
			}
			if($chat["sender_userid"] == $from_user_id) {
			$conversation .= '<form action="" method="POST">';	
			$conversation .= '<input type="hidden" required value=' . $chat["chatid"] . ' name="cid">';
			$conversation .= '<button style="float: left;" name="deleteChat" value="" title="Delete Message">Delete</button>';		
			$conversation .= '</form>';		
			}			
	...
	public function showUserChat($from_user_id, $to_user_id) {		
		...
		foreach ($userDetails as $user) {
			...
			$userSection = '<img src="images/'.$user['avatar'].'" alt="" />
				<p>'.$user['username'].'</p>

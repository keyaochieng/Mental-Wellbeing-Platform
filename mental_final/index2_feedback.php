...
$sql = "SELECT `feedback`.`Feedback_ID`, `feedback`.`User_ID`, `feedback`.`Details`, `feedback`.`Created_At`, `users`.`Fullname` FROM `feedback` JOIN `users` ON `users`.`User_ID` = `feedback`.`User_ID` WHERE `feedback`.`User_ID` = '$filter' OR `feedback`.`Appointment_ID` = '$filter2'";

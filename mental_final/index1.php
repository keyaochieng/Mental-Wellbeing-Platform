...
$sql = "SELECT `Appointment_ID`, `Details`, `Date`, `Time`, `Status` FROM `appointments` WHERE `Specialist_ID` = '$filter'";
...
?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to cancel this session?')?window.location.href='insertion.inc.php?action=cancelA&id=<?php echo($row["Appointment_ID"]); ?>':true;" title='Cancel Session'>Cancel</button></td>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to accept this session?')?window.location.href='insertion.inc.php?action=acceptA&id=<?php echo($row["Appointment_ID"]); ?>':true;" title='Accept Session'>Accept</button></td>
<?php
